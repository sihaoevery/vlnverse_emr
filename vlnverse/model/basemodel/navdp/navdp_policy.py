...
from vlnverse.model.encoder.navdp_backbone import *
...
class NavDPNet(PreTrainedModel):
    ...
    def predict_noise(self, last_actions, timestep, goal_embed, rgbd_embed):
        ...
        cond_embedding = torch.cat([time_embeds, goal_embed, rgbd_embed], dim=1) + self.cond_pos_embed(
    ...
    def predict_critic(self, predict_trajectory, rgbd_embed):
        ...
        cond_embeddings = torch.cat([nogoal_embed, nogoal_embed, repeat_rgbd_embed], dim=1) + self.cond_pos_embed(
            torch.cat([nogoal_embed, nogoal_embed, repeat_rgbd_embed], dim=1)
        )
    ...
    def forward(self, goal_point, goal_image, input_images, input_depths, output_actions, augment_actions):
        ...
        pg_output = self.decoder(
            tgt=pg_action_embeddings, memory=pg_cond_embeddings, tgt_mask=self.tgt_mask.to(ng_action_embeddings.device)
        )
