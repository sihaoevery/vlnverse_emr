...
class Renderer(abc.ABC):
    ...
    def render_offscreen(
        self, width: int, height: int, mode: RenderMode = RenderMode.RGB, camera_id: int = -1
    ) -> np.ndarray:
...
class MjPyRenderer(Renderer):
    ...
    def __init__(self, sim, **kwargs):
        assert isinstance(sim, module.get_mujoco_py().MjSim), 'MjPyRenderer takes a mujoco_py MjSim object.'
    ...
    def render_offscreen(
        ...
        if not self._offscreen_renderer:
            self._offscreen_renderer = module.get_mujoco_py().MjRenderContextOffscreen(self._sim)
...
class DMRenderer(Renderer):
    ...
    def __init__(self, physics, **kwargs):
        assert isinstance(physics, module.get_dm_mujoco().Physics), 'DMRenderer takes a DM Control Physics object.'
        super().__init__(**kwargs)
        ...
        if 'lookat' not in self._camera_settings:
            self._camera_settings['lookat'] = [np.median(self._physics.data.geom_xpos[:, i]) for i in range(3)]
    ...
    def render_offscreen(
        ...
        camera = mujoco.Camera(physics=self._physics, height=height, width=width, camera_id=camera_id)
        ...
        image = camera.render(depth=(mode == RenderMode.DEPTH), segmentation=(mode == RenderMode.SEGMENTATION))
...
class DMRenderWindow:
    ...
    def __init__(
        self, width: int = DEFAULT_WINDOW_WIDTH, height: int = DEFAULT_WINDOW_HEIGHT, title: str = DEFAULT_WINDOW_TITLE
    ):
        ...
        self._viewer = dmv.viewer.Viewer(self._viewport, self._window.mouse, self._window.keyboard)
    ...
    def load_model(self, physics):
        ...
        self._renderer = module.get_dm_viewer().renderer.OffScreenRenderer(physics.model, self._draw_surface)
