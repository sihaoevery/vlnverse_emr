...
class MujocoSimRobot:
    ...
    def __init__(self, model_file: str, use_dm_backend: bool = False, camera_settings: Optional[Dict] = None):
        ...
        if not os.path.isfile(model_file):
            raise ValueError('[MujocoSimRobot] Invalid model file path: {}'.format(model_file))
    ...
    def _patch_mjlib_accessors(self, model, data):
        ...
        def name2id(type_name, name):
            obj_id = mjlib.mj_name2id(model.ptr, mjlib.mju_str2Type(type_name.encode()), name.encode())
            if obj_id < 0:
