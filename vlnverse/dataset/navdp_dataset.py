...
class NavDP_Base_Datset(Dataset):
    ...
    def process_pixel_from_bytes(self, image_url, target_point, camera_intrinsic, camera_extrinsic):
        ...
        if pixel_coord_x >= image.shape[1]:
            pixel_mask = cv2.rectangle(
                pixel_mask, (image.shape[1] - 10, 0), (image.shape[1], image.shape[0]), (255, 255, 255), -1
            )
        if pixel_coord_y >= image.shape[0]:
            pixel_mask = cv2.rectangle(
                pixel_mask, (0, image.shape[0] - 10), (image.shape[1], image.shape[0]), (255, 255, 255), -1
            )
        if (
            pixel_coord_x > 0
    ...
    def __getitem__(self, index):
        ...
        memory_images, depth_image, memory_index = self.process_memory(
            self.trajectory_rgb_path[index], self.trajectory_depth_path[index], start_choice, memory_digit=memory_digit
        )
        target_local_points, augment_local_points, target_world_points, augment_world_points, action_indexes = (
            self.process_actions(
                trajectory_extrinsics, trajectory_base_extrinsic, start_choice, target_choice, pred_digit=pred_digit
            )
        )
        ...
        image_goal = np.concatenate(
            (
                self.process_image(self.trajectory_rgb_path[index][target_choice]),
                self.process_image(self.trajectory_rgb_path[index][start_choice]),
            ),
            axis=-1,
        )
        pixel_goal = self.process_pixel_from_bytes(
            self.trajectory_rgb_path[index][start_choice], pred_actions[-1], camera_intrinsic, trajectory_base_extrinsic
        )
